...
def encrypt(plain_text=None):
  key, fmt_key = create_key()
  plain_text = "This is very Important Message" if plain_text == None else plain_text
  pt_list = list(diffuse_pt(plain_text))
  mod = len(key)
  idx = 0
  enc_data = []
  for el in pt_list:
    enc_data.append(_xor(el, key[idx%mod]))
    idx += 1
  enc_data = ''.join(enc_data)
  return enc_data, cvt_key(enc_data), key, cvt_key(key)




if __name__ == '__main__':
	inp = str(input("anytxt: "))
	if inp == "":
		enc_dat , enc_text, keylist, key = encrypt()
	else:
		enc_dat , enc_text, keylist, key = encrypt(inp)
